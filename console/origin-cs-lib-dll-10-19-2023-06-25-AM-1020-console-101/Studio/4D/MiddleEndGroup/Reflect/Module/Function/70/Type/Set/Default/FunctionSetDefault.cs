...
namespace Core
{
    ...
    {
        public static IList<ReflectModuleFunctionSeventy> FunctionDefaultSet(String item_STRING, ReflectModuleFunctionSixty[] array_FUNCTION)
        {
            ICollection<ReflectModuleFunctionSeventy> collectionResult = default;

            collectionResult = new Collection<ReflectModuleFunctionSeventy>();

            var array = FunctionLayerContainerSurface<Tuple<ReflectModuleFunctionSixty[], ReflectModuleFunctionSixty[]>>(array_FUNCTION);
            ...
            foreach (var tuple in array)
            {
                ...
                {
                    var item_entry__FUNCTION = new ReflectModuleFunctionSeventy();
                    ...
                    foreach (var entry_FUNCTION in functionArrayNext)
                    {
                        ...
                        boolean_safe_CONTAIN_is = boolean_safe_CONTAIN_is && (entry_FUNCTION.Left.Position > item_FUNCTION.Left.Position) is true;

                        boolean_safe_CONTAIN_is = boolean_safe_CONTAIN_is && (entry_FUNCTION.Right.Position < item_FUNCTION.Right.Position) is true;
            ...
            foreach (var item_FUNCTION in collectionResult)
            {
                foreach (var entry_FUNCTION in collectionResult)
                {
                    ...
                    if (isNotReferenceCheck is true)
                    {
                        ...
                        boolean_safe_CONTAIN_is = boolean_safe_CONTAIN_is && (item_FUNCTION.Function.Left.Position < entry_FUNCTION.Function.Left.Position) is true;

                        boolean_safe_CONTAIN_is = boolean_safe_CONTAIN_is && (item_FUNCTION.Function.Right.Position > entry_FUNCTION.Function.Right.Position) is true;
            ...
            return new List<ReflectModuleFunctionSeventy>(collectionResult);
        }
    }
}
